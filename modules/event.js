...
const { promptForMessage, promptYesNo, pkQuery, getConfig } = require('../extras/common.js');
...
const TZ_MESSAGE_TEMPLATE = ({ tzlist }, config) => `\
**TIME ZONES**
      ...
      if (dueEvents.length > 0) {
        for (const event of dueEvents) {
          ...
          await destChannel.send(
            `The event **'${event.name}'** is starting now! <@&${event.role}>`, { embeds:
...
function embedEvent(event, guild, options = {}) {
  const config = getConfig(guild.client, guild.id);
  ...
  if (role && !firstRun) {
    eventEmbed.addField('Participants', `${role.members.size}`);
    if (forUser) {
      eventEmbed.addField(
        `Have you (${
          member.nickname ? member.nickname : member.user.username
...
async function createWizard(message, channel) {
  ...
  const embed = DMembedEvent(eventData, message.guild, {
...
module.exports = {
  name: 'event',
  description() { return 'Allows people on a server to participate in events';},
  usage(config) {
    return `create to start a DM session to create a new event
...
Staff can add users to the event by hand simply by giving any user the associated event role.`;
  ...
  async execute(message, args) {
    ...
    getConfig(message.client, message.guild.id);
