...
* TODO: voiceChamberSettings is undocumented added by previous project member.
...
const configurableProps = [{ varName:'prefix', shortDesc:'Command Prefix', varType:'prefix', default: '.', page:'main' },
  { varName:'roleAdmin', shortDesc:'Admin Role', varType:'role', default: '', page:'main' },
  { varName:'adminHoistToggle', shortDesc:'Toggle permitting staff to hoist to admin', varType:'boolean', default: false, page: 'main' },
  ...
  /* { varName:'invLogToggle', shortDesc:'Toggle __Invite Iogging__', varType:'boolean', default: false },
  { varName:'channelInvLogs', shortDesc:'Channel for logging joins/leaves', varType:'channel', default: '' },
  { varName:'knownInvites', shortDesc:'Invite Code Descriptions', varType:'inviteCodesArray', default: [] },
  { varName:'avatarLogToggle', shortDesc:'Toggle __avatar change__ logging', varType:'boolean', default: false },
  { varName:'channelAvatarLogs', shortDesc:'Channel for logging avatar changes', varType:'channel', default: '' },
  { varName:'avatarLogAirlockOnlyToggle', shortDesc:'Toggle __airlock exclusive__ avatar logging/reporting', varType:'boolean', default: false }, */
  ...
  { varName:'channelAnnouncements', shortDesc: 'Channel for making important announcements, such as admin escalations', varType:'channel', default: '', page:'specialChannels' },
];
...
async function generateEmbed(message, config, client, pageNo = 1) {
  ...
  const configEmbed = new MessageEmbed()
    .setTitle(`Server config for ${message.guild.me.displayName}`)
  ...
  configurableProps.forEach(prop =>{
    if (prop.page == pageArr[pageNo - 1]) {
      ...
      if (prop.varType == 'prefix') {
        ...
        const chanList = [];
        if (config[prop.varName].length > 0) {config[prop.varName].forEach(async chanId => chanList.push('#' + await getChannelName(chanId, client)));}
...
async function configDM(interaction, propToUpdate) {
  ...
  case 'prefix':
    msgContent += `Please reply with a new command prefix for ${interaction.guild.me.displayName} in ${interaction.guild.name}. Please note the following limitations:
  ...
  case 'role':
    ...
    break;
  case 'channelArray':
    if (config[propToUpdate.varName].length > 0) {config[propToUpdate.varName].forEach(async chanId => responseList.push('#' + await getChannelName(chanId, interaction.client)));}
  ...
  case 'inviteCodesArray':
    if (config[propToUpdate.varName].length > 0) { config[propToUpdate.varName].forEach(async invite => responseList.push(`${invite.code} - ${invite.description}`)); }
  ...
  await promptForMessage(dmChannel, async (reply) => {
    ...
    case 'boolean':
      ...
      case 'yes':
        ...
        dmChannel.send(`Ok. Setting ${propToUpdate.shortDesc} to ${config[propToUpdate.varName] ? 'OFF' : 'ON'}.`);
        return true;
      case 'cancel':
        return 'abort';
      case false:
